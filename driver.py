...
while loop:
    ...
    if (textArray[0] == "retract"):
        if currentUser is not None and currentUserType == "Author":
            retractManuscript(db, currentUser, textArray)
        else:
            print("Sorry, but you are either not logged in or an author.")
    if (textArray[0] == "submit"):
        if currentUser is not None and currentUserType == "Author":
            submitManuscript(db, currentUser, textArray)
        else:
            print("You must be logged in as an author to submit a Manuscript")
    # REGISTER
